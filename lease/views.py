...
@login_required
def download_file(request, obj_name):
    lease = LeaseRequest.objects.filter(
        user=request.user,
        status='APPROVED',
        expiration__gt=timezone.now()
    ...
    if not lease:
        return HttpResponse("No approved lease or lease expired.", status=403)
    ...
    try:
        s3_response = s3.get_object(Bucket=settings.S3_BUCKET_NAME, Key=obj_name)
        response = HttpResponse(s3_response['Body'].read(), content_type='application/octet-stream')
...
@login_required
def delete_file(request, obj_name):
    lease = LeaseRequest.objects.filter(
        user=request.user,
        status='APPROVED',
        expiration__gt=timezone.now()
    ...
    try:
        s3.delete_object(Bucket=settings.S3_BUCKET_NAME, Key=obj_name)
        return redirect('lease:s3_data')
    except Exception as e:
